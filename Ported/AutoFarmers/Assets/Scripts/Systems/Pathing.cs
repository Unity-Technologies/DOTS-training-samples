...
public static class Pathing 
{
	...
	public delegate bool IsNavigableDelegate(int x,int y, CommonSettings settings, DynamicBuffer<TileState> tiles);
	public delegate bool CheckMatchDelegate(int x,int y, CommonSettings settings, DynamicBuffer<TileState> tiles);
	...
	*/
	...
	public static bool IsNavigableAll(int x, int y, CommonSettings settings, DynamicBuffer<TileState> tiles) {
		return true;
	}

	public static bool IsNavigableDefault(int x, int y, CommonSettings settings, DynamicBuffer<TileState> tiles)
	{
		return (tiles[x + y * settings.GridSize.x].Value != ETileState.Rock);
	}

	public static bool IsRock(int x, int y, CommonSettings settings, DynamicBuffer<TileState> tiles)
	{
		return (tiles[x + y * settings.GridSize.x].Value == ETileState.Rock);
	}

	public static bool IsStore(int x, int y, CommonSettings settings, DynamicBuffer<TileState> tiles)
	{
		return (tiles[x + y * settings.GridSize.x].Value == ETileState.Store);
	}

	public static bool IsTillable(int x, int y, CommonSettings settings, DynamicBuffer<TileState> tiles)
	{
		return (tiles[x + y * settings.GridSize.x].Value == ETileState.Empty);
	}

	public static bool IsReadyForPlant(int x,int y, CommonSettings settings, DynamicBuffer<TileState> tiles) {
		return (tiles[x+ y * settings.GridSize.x].Value == ETileState.Tilled);
	}

	public static TileState FindNearbyRock(DynamicBuffer<PathNode> pathNodes, DynamicBuffer<TileState> tiles, int2 position, int range, CommonSettings settings) 
	{
		...
		if (rockPosHash == -1) {
			return new TileState { Value = ETileState.Empty};
		} else {
	...
	public static void WalkTo(DynamicBuffer<PathNode> pathNodes, DynamicBuffer<TileState> tileStates, int2 initPosition, int range, CommonSettings settings, CheckMatchDelegate CheckMatch)
	{
		NativeArray<int> visitedTiles;
	...
	public static int SearchForOne(int2 initPosition, int range, CommonSettings settings, DynamicBuffer<TileState> tileStates, IsNavigableDelegate IsNavigable, CheckMatchDelegate CheckMatch, RectInt requiredZone, out NativeArray<int> visitedTiles) {
	...
	public static NativeList<int> Search(int2 initPosition,int range, CommonSettings settings, DynamicBuffer<TileState> tileStates, IsNavigableDelegate IsNavigable,CheckMatchDelegate CheckMatch,RectInt requiredZone, out NativeArray<int> visitedTiles, int maxResultCount=0) {
