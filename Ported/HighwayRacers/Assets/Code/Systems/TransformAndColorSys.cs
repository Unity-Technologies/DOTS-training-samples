...
namespace HighwayRacer
{
    ...
    public class TransformAndColorSys : SystemBase
    {
        ...
        {
            prefabQuery = GetEntityQuery(typeof(CarPrefab));
            carQuery = GetEntityQuery(
                new EntityQueryDesc()
                {
                    All = new ComponentType[] {typeof(RenderMesh), typeof(LocalToWorld), typeof(URPMaterialPropertyBaseColor)},
        ...
        public static void setColor(in Car car, int entityIdx, ref NativeArray<URPMaterialPropertyBaseColor> colors)
        {
            ...
            {
                var percentage = (car.Speed - CarBucketSpawnSys.minSpeed) / (car.DesiredSpeedUnblocked - CarBucketSpawnSys.minSpeed);
        ...
        protected override void OnUpdate()
        {
            ...
            {
                carPrefab = prefabQuery.GetSingletonEntity();
                EntityManager.RemoveComponent<Rotation>(carPrefab);
                EntityManager.RemoveComponent<Translation>(carPrefab);
                EntityManager.AddComponent<URPMaterialPropertyBaseColor>(carPrefab);
                EntityManager.AddComponent<Prefab>(carPrefab);     // for some reason, the prefab entity doesn't have the prefab tag 
            }
            ...
            var chunks = carQuery.CreateArchetypeChunkArray(Allocator.TempJob); // todo : could this be persistent and created only upon respawning cars?
            var coreCount = JobsUtility.JobWorkerCount + 1;  // +1 for main thread 
            
            var nJobs = (chunks.Length >= coreCount) ? coreCount : chunks.Length;
            ...
            var bucketIdx = buckets.FirstNonEmptyBucketVisible();
            if (bucketIdx == -1)
            {
                Debug.Log("no non-empty buckets found! This shouldn't happen");
                return;
            }
            var firstCarIdx = 0;
            var firstChunkIdx = 0;
            for (int i = 0; i < nJobs; i++)
            {
                ...
                if (i < nJobs - 1)
                {
                    ...
                    buckets.NextNthCarVisible(ref bucketIdx, ref firstCarIdx, nCars);
    ...
    public struct TransformAndColorJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            var bucket = carBuckets.GetCarsVisible(bucketIdx);
            var segmentIdx = carBuckets.VisibleRoadSegmentIndex(bucketIdx);
            RoadSegment segment = roadSegments[segmentIdx];
            ...
            for (int chunkIdx = firstChunkIdx; chunkIdx <= lastChunkIdx; chunkIdx++)
            {
                ...
                for (int entityIdx = 0; entityIdx < chunk.Count; entityIdx++, carIdx++)
                {
                    while (carIdx >= bucket.Length)
                    {
                        carIdx = 0;
                        bucketIdx++;
                        bucket = carBuckets.GetCarsVisible(bucketIdx);
                        segmentIdx = carBuckets.VisibleRoadSegmentIndex(bucketIdx);
                        segment = roadSegments[segmentIdx];
                    }

                    Car car = bucket[carIdx];

                    TransformAndColorSys.setTransform(ref car, entityIdx, ref transforms, ref segment);
                    TransformAndColorSys.setColor(in car, entityIdx, ref colors);
                }
            }
        }
    }
