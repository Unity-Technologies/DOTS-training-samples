...
public class TrackInitialisationSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        Entities
            ...
            {
                var tracks = GetComponentDataFromEntity<TrackProperties>(true);
                var track = tracks[spawner.TrackPrefab];

                for (int i = 0; i < track.NumberOfLanes; ++i)
                {
                    ...
                    var position = new float3(
                        track.TrackStartingPoint.x,
                        track.TrackStartingPoint.y + (track.LaneWidth + track.SeparationWidth) * i,
                        40);
                    ...
                    var scale = new float3(track.TrackLength, track.LaneWidth, 1);
