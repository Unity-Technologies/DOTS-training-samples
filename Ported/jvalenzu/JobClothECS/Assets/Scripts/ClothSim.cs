...
public class ClothSim : MonoBehaviour {
    ...
    PerFrameInitJob perFrameInitJob;
    JobHandle perFrameInitHandle;
    ...
    struct PerFrameInitJob : IJobParallelForBatch {
	public NativeArray<int> timesTouched;

        public void Execute(int startIndex, int count) {
	    for (int i=0; i<count; ++i)
		timesTouched[startIndex+i] = 0;
        }
    };
    ...
    struct BarJob:IJob {
        ...
        public void Execute() {
            for (int i = 0; i < bars.Length; i++) {
                ...
                timesTouched[pair.x] = timesTouched[pair.x]+1;
                timesTouched[pair.y] = timesTouched[pair.y]+1;
	...
	processorCount = SystemInfo.processorCount;
        ...
        // keep track of connectivity of mesh to get a better idea how feasible it is
	// to simulate in chunks
        timesTouched = new NativeArray<int>(mesh.vertices.Length, Allocator.Persistent);

        int perFrameChunkSize = timesTouched.Length/processorCount;
    ...
    void Update () {
	    ...
	    Vector2[] uvs = new Vector2[timesTouched.Length];

	    int mn = 10000;
	    int mx = 0;
	    for (int i=0; i<timesTouched.Length; ++i)
	    {
		int c = timesTouched[i];
		if (mn > c) mn = c;
		if (mx < c) mx = c;
	    }
	    ...
	    for (int i=0; i<timesTouched.Length; ++i)
	    {
		float v = (timesTouched[i]-mn)/(float)mx;
		uvs[i] = new Vector2(v,v);
	    }
	    mesh.uv = uvs;
        }

        perFrameInitJob = new PerFrameInitJob {
        ...
        perFrameInitHandle = perFrameInitJob.ScheduleBatch(timesTouched.Length, timesTouched.Length/processorCount);
        barJobHandle = barJob.Schedule(perFrameInitHandle);
