...
public class ClothSimForEach : JobComponentSystem
{
...
#if UNITY_EDITOR
    static Dictionary<Mesh, int> meshToBlobAssetReferenceIndexLookup = new Dictionary<Mesh, int>();
    static BlobAssetReference<ClothConstraintAsset>[] blobAssets;
    static int blobAssetCount;

    protected override void OnCreate()
    {
	blobAssetCount = 0;
	blobAssets = new BlobAssetReference<ClothConstraintAsset>[(int)ArbitraryResourceConstants.kMaxBlobAssets];
	meshToBlobAssetReferenceIndexLookup = new Dictionary<Mesh, int>();
    }
    ...
    static public BlobAssetReference<ClothConstraintAsset> AddBlobAssetReference(Entity entity, Mesh mesh)
    {
        if (!meshToBlobAssetReferenceIndexLookup.ContainsKey(mesh))
        {
            ...
            for (int i=0,n=barList.Count; i<n; ++i) {
                ...
                float3 p1 = mesh.vertices[pair.x];
                float3 p2 = mesh.vertices[pair.y];
                ...
                constraint.length = (ushort) (math.length(p2 - p1) * 256);
            ...
            meshToBlobAssetReferenceIndexLookup.Add(mesh, index);

            builder.Dispose();
        }

        return blobAssets[meshToBlobAssetReferenceIndexLookup[mesh]];
    }
#else
#endif
    ...
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        ...
        float4 worldGravity = new float4(-math.up() * Time.DeltaTime*Time.DeltaTime, 0.0f);
	...
	//  1 |  0 |  0 |  2
	//  1 |  1 |  0 |  0
	NativeArray<float> localScaleX = new NativeArray<float>(new float[]{1.0f, 2.0f, 0.0f, 0.0f}, Allocator.TempJob);
	NativeArray<float> localScaleY = new NativeArray<float>(new float[]{1.0f, 0.0f, 2.0f, 0.0f}, Allocator.TempJob);
                      ...
                      in LocalToWorld localToWorld) =>
            {
                ...
                for (int i=0,n=pins.Length; i<n; ++i)
                {
                    ...
                    if (v1.y < clothInstance.localY0) {
                        ...
                        float3 oldWorldPos = math.transform(localToWorld.Value, v0);
