...
namespace Original
{
    public class FireSim : MonoBehaviour
    {
        ...
        [Header("WATER")] [Range(1, 5)] [Tooltip("Number of cells affected by a bucket of water")]
        ...
        [Range(0, 100)] public int totalBuckets = 3;
        ...
        [Header("FIRE")] [Tooltip("Prefabs / FlameCell")]
        ...
        [HideInInspector] public FlameCell[] flameCells;
        [HideInInspector] public int cellsOnFire;
        [HideInInspector] public int totalCells;
        private float timeUntilFireUpdate;

        [Header("BOTS")] public GameObject prefab_bucket;
        public GameObject prefab_bot;
        [HideInInspector] public List<BucketChain> bucketChains;
        public List<BucketChain_Config> bucketChain_Configs;
        [Range(0.0001f, 1f)] public float botSpeed = 0.1f;
        [Range(0.001f, 1f)] public float waterCarryAffect = 0.5f;
        public int total_OMNIBOTS = 0;
        private int botCount = 0;
        private List<Bot> allBots;
        [HideInInspector] public List<Bucket> allBuckets;
        [HideInInspector] public List<Water> allWater;
        ...
        void PopulateFireGrid()
        {
            ...
            for (int rowIndex = 0; rowIndex < rows; rowIndex++)
            {
                for (int columnIndex = 0; columnIndex < columns; columnIndex++)
                {
                    ...
                    GameObject _tempFlame =
                        (GameObject)Instantiate(prefab_flameCell, transform.position, transform.rotation);
                    flameCells[cellIndex] = _tempFlame.GetComponent<FlameCell>();
                    flameCells[cellIndex].Init(transform, cellIndex, rowIndex, columnIndex, cellSize, maxFlameHeight,
                        colour_fireCell_neutral);
                    cellIndex++;
                }
            }
        }
            ...
            for (int rowIndex = -splashRadius; rowIndex <= splashRadius; rowIndex++)
            {
                ...
                if (currentRow >= 0 && currentRow < rows)
                {
                    for (int columnIndex = -splashRadius; columnIndex <= splashRadius; columnIndex++)
                    {
                        ...
                        if (currentColumn >= 0 && currentColumn < columns)
                        {
                            float dowseCellStrength = 1f / (Mathf.Abs(rowIndex * coolingStrength_falloff) +
                                                            Mathf.Abs(columnIndex * coolingStrength_falloff));
                            flameCells[(currentRow * columns) + currentColumn]
                                .Extinguish((coolingStrength * dowseCellStrength) * bucketCapacity);
                        }
                    }
                }
            }
        ...
        void Setup_Buckets()
        {
            ...
            for (int i = 0; i < totalBuckets; i++)
            {
                ...
                _tempBucket.Init(allBuckets.Count, Random.Range(0f, simulation_WIDTH),
                    Random.Range(0f, simulation_DEPTH));
        ...
        public Bot AddBot(BotType _botType, BucketChain _parentChain = null)
        {
            ...
            // initialise bot to desired type
            _tempBot.Init(_botType, allBots.Count, Random.Range(0f, simulation_WIDTH),
                Random.Range(0f, simulation_DEPTH));
