...
namespace Original
{
    public class FlameCell : MonoBehaviour
    {
        ...
        [Range(0.001f, 10f)] public float flickerRate = 0.1f;
        [Range(0f, 1f)] public float flickerRange = 0.1f;
        ...
        public void Init(Transform _parent, int _index, int _rowIndex, int _columnIndex, float _size, float _maxHeight,
            Color _col)
        {
            t = transform;
            t.parent = _parent;
            ...
            // position
            t.position = new Vector3(_size * _rowIndex, -(_maxHeight * 0.5f) + Random.Range(0.01f, 0.02f),
                _columnIndex * _size);
            mat.color = _col;
        }
        ...
        void Update()
        {
            if (onFire)
            {
                ...
                _POS.y += (flickerRange * 0.5f) +
                          Mathf.PerlinNoise((Time.time - index) * flickerRate - temperature, temperature) *
                          flickerRange;
                t.localPosition = _POS;
