...
namespace Original
{
    ...
    public class Templates : MonoBehaviour
    {
        ...
        void PrintSummary()
        {
            ...
            Debug.Log("splashRadius: " + fs.splashRadius + "| coolingStrength: " + fs.coolingStrength +
                      "| coolingStrength_falloff: " + fs.coolingStrength_falloff);
            Debug.Log("Fires:" + fs.startingFireCount + " | flameHeight:" + fs.maxFlameHeight + " | cellSize:" +
                      fs.cellSize + " | rows:" + fs.rows + " | columns:" + fs.columns);
            Debug.Log("flashpoint:" + fs.flashpoint + " | heatRadius:" + fs.heatRadius + " | heatTransferRate:" +
                      fs.heatTransferRate);
            Debug.Log("Buckets:" + fs.totalBuckets + " | bucketCapacity:" + fs.bucketCapacity + " | bucketFillRate:" +
                      fs.bucketFillRate + " | waterCarryAffect:" + fs.waterCarryAffect);
            ...
            for (int i = 0; i < fs.bucketChain_Configs.Count; i++)
            {
                ...
                Debug.Log("Chain " + i + ": passers_EMPTY [" + _Config.passers_EMPTY + "], passers_FULL[" +
                          _Config.passers_FULL + "]");
            }
