...
partial struct WaterBringerFindNewTarget : IJobEntity
{
    ...
    void Execute( ref LineId lineId, ref LineIndex lineIndex, ref Position target)
    {
        ...
        pos = StartPositions[lineId.Value] + multiplier * (EndPositions[lineId.Value] - StartPositions[lineId.Value]);
...
[BurstCompile]
partial struct WaterBringerAISystem : ISystem
{
    [BurstCompile]
    public void OnCreate(ref SystemState state)
    {
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach (var (FireFighterDefaultPos, FireFighterPos, target, FireFighterBucketId, lineId, lineIndex, WaterBringerState) in SystemAPI.Query<RefRW<Position>, RefRW<Translation>, RefRW<Target>, RefRW<BucketId>, RefRO<LineId>, RefRO<LineIndex>, RefRW<WaterBringer>>().WithAll<WaterBringer>())
        {
