...
partial struct FireSimSystem : ISystem
{
    ...
    {
        var gridTemperatures = SystemAPI.GetSingletonRW<GridTemperatures>();
        ...
        // update rate
        if (currentTime <= gridTemperatures.ValueRO.NextGridUpdateTime) return;

        float timeSinceLastFireSim = (float)(currentTime - gridTemperatures.ValueRO.LastGridUpdateTime);
        gridTemperatures.ValueRW.LastGridUpdateTime = currentTime;

        // writes temperatures for fire spreading, increase temperature over time
        gridTemperatures.ValueRW.NextGridUpdateTime = currentTime + heatTransferRate;
        var baseHeatIncreaseRate = config.baseHeatIncreaseRate;
        for (int j = 0; j < config.gridSize; j++)
        {
            for (int i = 0; i < config.gridSize; i++)
            {
                var currentTemp = gridTemperatures.ValueRO.Get(i, j);
                if (currentTemp > 0)
                {
                    ...
                    float UpdateNeighbourAndGetBonus(int i, int j, float currentTemp)
                    {
                        ...
                        var neighbourTemp = gridTemperatures.ValueRO.Get(i, j);
                        if (neighbourTemp == 0 && currentTemp > k_OnFireTemp)
                        {
                            gridTemperatures.ValueRW.Set(i, j, 0.05f);
                            return 0f;
                        }
                        else
                        {
                    ...
                    var ownHeatChange = timeSinceLastFireSim * baseHeatIncreaseRate * bonus;
                    var newHeat = math.min(currentTemp + ownHeatChange, k_MaxHeat);
                    gridTemperatures.ValueRW.Set(i, j, newHeat);
                }
            }
        }
        ...
        foreach (var fireCell in SystemAPI.Query<FireCellAspect>())
        {
            ...
            var temp = gridTemperatures.ValueRO.Get(fireCell.CellInfo.ValueRO.indexX, fireCell.CellInfo.ValueRO.indexY);
