...
public class HomebaseGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
    struct UpdateInterpolatedJob : IJobChunk
    {
        [ReadOnly] public NativeHashMap<int, GhostEntity> GhostMap;
        [ReadOnly] public ArchetypeChunkBufferType<HomebaseSnapshotData> ghostSnapshotDataType;
        [ReadOnly] public ArchetypeChunkEntityType ghostEntityType;
        public ArchetypeChunkComponentType<HomebaseComponent> ghostHomebaseComponentType;
        ...
        public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
        {
            ...
            var ghostHomebaseComponentArray = chunk.GetNativeArray(ghostHomebaseComponentType);
            ...
            for (int entityIndex = 0; entityIndex < ghostEntityArray.Length; ++entityIndex)
            {
                ...
                HomebaseSnapshotData snapshotData;
                snapshot.GetDataAtTick(targetTick, targetTickFraction, out snapshotData);

                var ghostHomebaseComponent = ghostHomebaseComponentArray[entityIndex];
                var ghostRotation = ghostRotationArray[entityIndex];
                var ghostTranslation = ghostTranslationArray[entityIndex];
                ghostHomebaseComponent.Color = snapshotData.GetHomebaseComponentColor(deserializerState);
                ghostHomebaseComponent.PlayerId = snapshotData.GetHomebaseComponentPlayerId(deserializerState);
                ...
                ghostHomebaseComponentArray[entityIndex] = ghostHomebaseComponent;
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<HomebaseSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadWrite<HomebaseComponent>(),
        ...
        {
            All = new []{
                ComponentType.ReadOnly<HomebaseSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
        ...
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<HomebaseSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        if (!m_predictedQuery.IsEmptyIgnoreFilter)
        {
            var updatePredictedJob = new UpdatePredictedJob
            {
                ...
                ghostSnapshotDataType = GetArchetypeChunkBufferType<HomebaseSnapshotData>(true),
                ...
                ghostHomebaseComponentType = GetArchetypeChunkComponentType<HomebaseComponent>(),
...
public partial class HomebaseGhostSpawnSystem : DefaultGhostSpawnSystem<HomebaseSnapshotData>
{
}
