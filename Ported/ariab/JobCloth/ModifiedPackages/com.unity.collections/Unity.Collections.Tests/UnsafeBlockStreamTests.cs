...
public class UnsafeBlockStreamTests
{
    struct WriteInts : IJobParallelFor
    {
        public UnsafeBlockStream.Writer Writer;
    ...
    {
        public UnsafeBlockStream.Reader Reader;
    ...
    {
        var stream = new UnsafeBlockStream(count);
    ...
    {
        var stream = new UnsafeBlockStream(100);
        var fillInts = new WriteInts {Writer = stream.AsWriter()};
    ...
    public void ScheduleCreateThrows()
    {
        ...
        UnsafeBlockStream stream;
        var jobHandle = UnsafeBlockStream.ScheduleConstruct(out stream, list, default(JobHandle));
    ...
    {
        var stream = new UnsafeBlockStream(1);
    ...
    {
        var stream = new UnsafeBlockStream(2);
        var writer = stream.AsWriter();
        writer.BeginForEachIndex(0);
    ...
    static void CreateBlockStream1And2Int(out UnsafeBlockStream stream)
    {
        stream = new UnsafeBlockStream(2);
