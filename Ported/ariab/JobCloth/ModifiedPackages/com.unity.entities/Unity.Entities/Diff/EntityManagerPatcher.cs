...
namespace Unity.Entities
{
    ...
    public struct EntityManagerPatcher : IDisposable
    {
        ...
        private readonly EntityQuery m_EntityQuery;
        private readonly EntityQuery m_PrefabQuery;
        private readonly EntityQuery m_LinkedEntityGroupQuery;
    ...
    internal static class EntityManagerPatcherUtility
    {
            ...
            where TComponent : struct, IEquatable<TComponent>, IComponentData
        {
            ...
            [WriteOnly] public NativeMultiHashMap<TComponent, Entity>.Concurrent ComponentToEntity;
            ...
            where TComponent : struct, IEquatable<TComponent>, IComponentData
        {
            ...
            [WriteOnly] public NativeHashMap<TComponent, Entity>.Concurrent ComponentToEntity;
            ...
            where TComponent : struct, IEquatable<TComponent>, IComponentData
        {
            ...
            [WriteOnly] public NativeHashMap<Entity, TComponent>.Concurrent EntityToEntityGuid;
        ...
        {
            [WriteOnly] public NativeHashMap<Entity, Entity>.Concurrent EntityToLinkedEntityGroupRoot;
            [ReadOnly] public ArchetypeChunkBufferType<LinkedEntityGroup> LinkedEntityGroupType;
        ...
        private struct BuildPackedEntityLookupJob : IJobParallelFor
        {
            ...
            [WriteOnly] public NativeMultiHashMap<int, Entity>.Concurrent PackedEntities;
        ...
        {
            [ReadOnly] public NativeArray<ulong> TypeHashes;
            [WriteOnly] public NativeArray<ComponentType> PackedTypes;
            
            public void Execute(int index)
            {
                var stableTypeHash = TypeHashes[index];
            ...
            EntityChangeSet changeSet)
        {
            ...
            var entityCount = entityQuery.CalculateLength();
            ...
            using (var entityToEntityGuid = new NativeHashMap<Entity, EntityGuid>(entityQuery.CalculateLength(), Allocator.TempJob))
            {
                BuildEntityLookups(
                    entityManager, 
                    entityQuery, 
                ...
                using (var entityGuidToPrefab = new NativeHashMap<EntityGuid, Entity>(prefabQuery.CalculateLength(), Allocator.TempJob))
            ...
            NativeHashMap<Entity, EntityGuid> entityToEntityGuid)
        {
            var buildEntityGuidToEntity = new BuildComponentToEntityMultiHashMap<EntityGuid>
            {
                ...
                ComponentToEntity = entityGuidToEntity.ToConcurrent()
            ...
            var buildEntityToEntityGuid = new BuildEntityToComponentHashMap<EntityGuid>
            {
                ...
                EntityToEntityGuid = entityToEntityGuid.ToConcurrent()
            }.Schedule(entityQuery);
            ...
            NativeHashMap<Entity, Entity> entityToLinkedEntityGroupRoot)
        {
            ...
            var buildPrefabLookups = new BuildComponentToEntityHashMap<EntityGuid>
            {
                EntityType = entityManager.GetArchetypeChunkEntityType(),
                ComponentType = entityManager.GetArchetypeChunkComponentType<EntityGuid>(true),
                ComponentToEntity = entityGuidToPrefab.ToConcurrent()
            }.Schedule(prefabQuery);
                
            var buildLinkedEntityGroupLookups = new BuildLinkedEntityGroupHashMap
            {
                EntityToLinkedEntityGroupRoot = entityToLinkedEntityGroupRoot.ToConcurrent(),
            ...
            NativeArray<ComponentType> packedTypes)
        {
            var buildPackedEntityLookups = new BuildPackedEntityLookupJob
            {
                ...
                PackedEntities = packedEntities.ToConcurrent()
            ...
            NativeMultiHashMap<EntityGuid, Entity> entityGuidToEntity)
        {
            for (var i = changeSet.Entities.Length - changeSet.DestroyedEntityCount; i < changeSet.Entities.Length; i++)
            {
                ...
                do
                {
                    ...
                    entityGuidToEntity.TryRemove(changeSet.Entities[i], entity);
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (componentTypeInArchetype.TypeIndex == entityGuidTypeIndex)
                                {
                                    ...
                                    {
                                        entityGuidToEntity.TryRemove(entityGuid, entity);
                                    }
            ...
            using (var removals = new NativeList<Child>(Allocator.TempJob))
            {
                ...
                for (var i = 0; i < removals.Length; ++i)
                {
                    ...
                    for (var packedEntityGuidIndex = 0; packedEntityGuidIndex < packedEntityGuids.Length; ++packedEntityGuidIndex)
                    {
                        ...
                        {
                            packedEntities.TryRemove(packedEntityGuidIndex, removal.ChildEntity);
                            break;
                        }
                    }
                    
                    entityToEntityGuid.Remove(removal.ChildEntity);
                    entityGuidToEntity.TryRemove(removal.ChildEntityGuid, removal.ChildEntity);
