    ...
    class SubSceneLiveLinkSystem : ComponentSystem
    {
        ...
        void ApplyLiveLink(SubScene scene)
        {
            ...
            if (scene.LiveLinkShadowWorld == null)
            {
                ...
                scene.LiveLinkPatcher = new EntityManagerPatcher(World, Allocator.TempJob);
            }
            ...
            using (var cleanConvertedEntityWorld = new World("Clean Entity Conversion World"))
            {
                ...
                using (var changes = EntityManagerDifferUtility.GetChanges(
                    cleanConvertedEntityWorld.EntityManager, 
                    scene.LiveLinkShadowWorld.EntityManager, 
                    EntityManagerDifferOptions.IncludeForwardChangeSet|EntityManagerDifferOptions.FastForwardShadowWorld|EntityManagerDifferOptions.ClearMissingReferences, Allocator.TempJob))
                {
                    scene.LiveLinkPatcher.ApplyChangeSet(changes);
                }
