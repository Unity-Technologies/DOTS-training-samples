...
public partial class PointSimulationSystem : SystemBase
{
	...
	protected override void OnUpdate()
    {
        ...
        BufferFromEntity<CurrentPoint> allCurrentPoints
            = this.GetBufferFromEntity<CurrentPoint>(false);

		BufferFromEntity<PreviousPoint> allPreviousPoints
		   = this.GetBufferFromEntity<PreviousPoint>(false);
		...
		Entities
			.ForEach((Entity entity, in World world) => 
        {
			DynamicBuffer<CurrentPoint> currentPoints = allCurrentPoints[entity];
			DynamicBuffer<PreviousPoint> previousPoints = allPreviousPoints[entity];
			...
			for (int i = 0; i < currentPoints.Length; i++)
			{
				...
				//	if (point.anchor == false)
				{
					...
					allCurrentPoints[entity].ElementAt(i).Value = point;
					allPreviousPoints[entity].ElementAt(i).Value = previousPoint;
