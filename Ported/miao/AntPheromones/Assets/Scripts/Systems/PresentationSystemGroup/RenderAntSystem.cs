    ...
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        ...
        for (int batch = 0; batch < numBatches; batch++)
        {
            ...
            NativeArray<Matrix4x4>.Copy(
                src: localToWorlds.GetSubArray(start: batch * Rendering.MaxNumMeshesPerDrawCall, length: batchSize),
                dst: this._matrices[batch],
                length: batchSize);
            NativeArray<Vector4>.Copy(
                src: colours.GetSubArray(start: batch * Rendering.MaxNumMeshesPerDrawCall, length: batchSize),
