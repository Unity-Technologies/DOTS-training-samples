    ...
    class SceneSectionStreamingSystem : ComponentSystem
    {
                ...
                using (var sceneEntities = sceneDataGrp.ToEntityArray(Allocator.TempJob))
                {
                    foreach (var sceneEntity in sceneEntities)
                    {
                        ...
                        if (scenedata.SceneGUID == guid && scenedata.SubSectionIndex == 0)
                        {
                            ...
                            {
                                group.SetSharedComponentFilter(new SceneTag { SceneEntity = sceneEntity });
                    ...
                    if (operation.IsCompleted && moveEntitiesFromProcessed < MaximumMoveEntitiesFromPerFrame)
                    {
                        ...
                        if (operation.ErrorStatus == null)
                        {
                            ...
                            if (EntityManager.HasComponent<RequestSceneLoaded>(m_Streams[i].SceneEntity))
                            {
                                ...
                                {
                                    var bundle = m_Streams[i].Operation.StealBundle();
                                    ...
                                    if (bundle != null)
                                        EntityManager.AddSharedComponentData(m_Streams[i].SceneEntity, new SceneSectionBundle(bundle));
        ...
        int CreateAsyncLoadScene(Entity entity)
        {
            for (int i = 0; i != m_Streams.Length; i++)
            {
                ...
                m_Streams[i].Operation = new AsyncLoadSceneOperation(entitiesBinaryPath, sceneData.FileSize, sceneData.ObjectReferenceCount, resourcesPath, entityManager, useAssetBundle);
