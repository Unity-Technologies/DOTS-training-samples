		...
		for (int i = 0; i < bees.Count; i++)
		{
			...
			if (bee.dead == false)
			{
				...
				else if (bee.enemyTarget != null)
				{
					...
					else
					{
						...
						else
						{
							...
							{
								ParticleManager.SpawnParticle(bee.enemyTarget.position, ParticleType.Blood,
									bee.velocity * .35f, 2f, 6);
				...
				else if (bee.resourceTarget != null)
				{
					...
					{
						Vector3 targetPos = new Vector3(-Field.size.x * .45f + Field.size.x * .9f * bee.team, 0f,
							bee.position.z);
