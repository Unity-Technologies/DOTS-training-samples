...
public class MouseRaycaster : MonoBehaviour {
	...
	void LateUpdate () {
		...
		for (int i=0;i<3;i++) {//this for runs 3 times 
			for (int j=-1;j<=1;j+=2) {// this for runs twice with j -1 and 1 
				...
				//dot product returns values greater than 0 when the angle between two vectors is less than 90 degrees 
				...
				if (Vector3.Dot(plane.normal,mouseRay.direction) < 0f) {
					if (plane.Raycast(mouseRay,out hitDistance)) {
						...
						if (insideField) {//if all 3 variables of the vector are inside the field it goes into this if  
