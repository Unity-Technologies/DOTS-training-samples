...
public class DrawingSystem : ComponentSystem
{
    ...
    protected override void OnUpdate()
    {
        if (!Highway.USE_HYBRID_RENDERER)
        {
            ...
            {
                NativeArray<Matrix4x4>.Copy(positions.Reinterpret<Matrix4x4>().GetSubArray(batch * 1023, 1023),
                    matrices, 1023);
                NativeArray<Vector4>.Copy(colors.Reinterpret<Vector4>().GetSubArray(batch * 1023, 1023),
            ...
            if (rest > 0)
            {
                NativeArray<Matrix4x4>.Copy(positions.Reinterpret<Matrix4x4>().GetSubArray(numBatches * 1023, rest),
                    matrices, rest);
                NativeArray<Vector4>.Copy(colors.Reinterpret<Vector4>().GetSubArray(numBatches * 1023, rest),
